...
public class ApplicationExportTask implements ExportTask {
  ...
  public void export(OutputStream outputStream) throws IOException {
    ...
    try {
      application = applicationRegistryService.getApplication(categoryName, applicationName);
    } catch (Exception e1) {
      throw new OperationException(OperationNames.EXPORT_RESOURCE, "Error while exporting application data: " + getEntry(), e1);
    }
    if (application == null) {
