...
public class SelectNodesPopupComponent extends UIForm implements UIPopupComponent {
  ...
  public static String[] COMPARAISON_BEAN_FIELD = { "title", "path", "lastModifierUserName", "sourceModificationDate", "targetModificationDate", "stateLocalized" };
  ...
  public SelectNodesPopupComponent() throws Exception {
    ...
    stateCheckBoxInput.setValue(NodeComparaisonState.MODIFIED_ON_SOURCE.getKey());
  ...
  public void computeComparaisons() {
    ...
    filterString = getUIStringInput(FILTER_CONTENT_FIELD_NAME).getValue();
    modifiedDateFilter = getUIFormDateTimeInput(PUBLICATION_DATE_FIELD_NAME).getCalendar();
    stateString = getUIFormSelectBox(CONTENT_STATE_FIELD_NAME).getValue();
    publishedContentOnly = getUICheckBoxInput(PUBLISHED_CONTENT_ONLY_FIELD_NAME).getValue();
